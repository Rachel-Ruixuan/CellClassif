...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("phase2.log")  # Logs to a file
    ]
)
        ...
        for track_id, track in tqdm(tracking_data.items(), desc="Processing tracks"):
            ...
            for point in track:
                ...
                if isinstance(point['segmentation'], list) and len(point['segmentation']) > 0:
                    # Assuming 'segmentation' is a list of lists (for multiple polygons)
                    # and we take the first polygon
                    poly = np.array(point['segmentation'][0]).reshape(-1, 2)
